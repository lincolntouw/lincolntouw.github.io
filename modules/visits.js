...
async function getGames(userId, cursor) {
  const allGames = [];         
  const list = await(await fetch( proxy + encodeURIComponent(`https://games.roblox.com/v2/users/${userId}/games?sortOrder=Asc&limit=50${cursor != undefined ? `&cursor={cursor}` : ""}`) )).json();
